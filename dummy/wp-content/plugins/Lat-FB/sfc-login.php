...
?>
	<table class="form-table">
		<tr>
			<th><label><?php _e('Tích hợp facebook', 'sfc'); ?></label></th>
<?php
	$fbuid = get_user_meta($profile->ID, 'fbuid', true);
	if (empty($fbuid)) {
		?>
			<td><p><fb:login-button scope="email" v="2" size="large" onlogin="sfc_login_update_fbuid(0);"><?php _e('Kết Nối Facebook', 'sfc'); ?></fb:login-button></p></td>
		</tr>
	</table>
	<?php
	} else { ?>
		<td><p><?php _e('Tích hợp với', 'sfc'); ?>
...
<?php if (SFC_ALLOW_DISCONNECT) { ?>
		<input type="button" class="button-primary" value="<?php _e('Ngắt kết nối', 'sfc'); ?>" onclick="sfc_login_update_fbuid(1); return false;" />
<?php } ?>
		</p></td>
	<?php } ?>
	</tr>
	...
	if($fbuid) {
		...
		if ($user_id) {
			...
			if (!$user_id) {
				...
				global $error;
				$error = '<strong>'.__('LỖI', 'sfc').'</strong>: '.__('Không đăng nhập được!! Không có tài khoản nào tích hợp với facebook này.', 'sfc');
			}
		}
	}
...
function sfc_login_admin_init() {
	...
	add_settings_field('sfc_login_avatars', __('Dùng Avartar Facebook', 'sfc'), 'sfc_login_avatar_callback', 'sfc', 'sfc_login');
}


function sfc_login_section_callback() {
	echo "<p>".__('Cài đặt cho module đăng nhập. Người dùng có thể kết nối tài khoản của họ tới facebook thông qua menu Users->Your Profile screen.', 'sfc')."</p>";
}
...
function sfc_login_avatar_callback() {
	...
	?>
	<p><input type="checkbox" name="sfc_options[login_avatars]" value="1" <?php checked('1', $options['login_avatars']); ?> /><label> <?php _e('Dùng Facebook Avatars thay cho Gravatars','sfc'); ?></label>	</p>
<?php
}
