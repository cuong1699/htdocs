...
$sfc_plugin_descriptions = array(
	'plugin_login'		=>__('Đăng nhập với facebook','sfc'),
	'plugin_register'	=>__('Đăng ký với facebook ( Phải kích hoạt luôn cả đăng nhập)','sfc'),
...
function sfc_admin_init(){
	...
	if (empty($options['app_secret']) || empty($options['appid'])) {
		add_action('admin_notices', create_function( '', "echo '<div class=\"error\"><p>".sprintf(__('Để tích hợp facebook bạn cần cài đặt các thông tin cần thiết cho Plugin! Vào <a href="%s">settings</a> page.', 'sfc'), admin_url('options-general.php?page=sfc'))." để cài đặt</p></div>';" ) );
	} else {
	...
	add_settings_section('sfc_main', __('Tùy chỉnh chính', 'sfc'), 'sfc_section_text', 'sfc');
	...
	add_settings_section('sfc_plugins', __('Modules tích hợp', 'sfc'), 'sfc_plugins_text', 'sfc');
...
function sfc_admin_add_page() {
	...
	$sfc_options_page = add_options_page(__('Tích hợp facebook', 'sfc'), __('Tích hợp facebook', 'sfc'), 'manage_options', 'sfc', 'sfc_options_page');
...
?>
	<div class="wrap">
	<?php screen_icon(); ?>
	<h2><?php _e('Tích hợp facebook', 'sfc'); ?></h2>
	<p><?php _e('Các cài đặt liên quan tới Plugin tích hợp facebook', 'sfc'); ?> </p>
...
function sfc_section_text() {
	...
	if (empty($options['app_secret']) || empty($options['appid'])) {
?>
<p><?php _e('Để tích hợp Facebook bạn sẽ cần tạo 1 App Facebook. Nếu bạn đã tạo 1 ứng dụng thì điền App ID và App Secret key vào dưới đây.', 'sfc'); ?></p>
<p><strong><?php _e('Không tìm thấy key?', 'sfc'); ?></strong></p>
<ol>
<li><?php _e('Các ứng dụng mà bạn đã tạo <a target="_blank" href="https://developers.facebook.com/apps">Facebook Application List</a>', 'sfc'); ?></li>
<li><?php _e('Chọn ứng dụng bạn dùng để tích hợp facebook vào wordpress sau đó copy App Id và App secret key và dán vào dưới.', 'sfc'); ?></li>
</ol>

<p><strong><?php _e('Bạn chưa tạo ứng dụng?', 'sfc'); ?></strong> <?php _e('Don\'t worry, it\'s easy!', 'sfc'); ?></p>
<ol>
<li><?php _e('Vào link sau để tạo 1 App: <a target="_blank" href="https://developers.facebook.com/apps">Facebook Application Setup</a>', 'sfc'); ?></li>
<li><?php $home = home_url('/'); _e("Sau khi tạo App, điền <strong>{$home}</strong> vào mục the Site URL trong phần website login.", 'sfc'); ?></li>
<li><?php _e('Bạn có thể lấy các API tại 
<a target="_blank" href="https://developers.facebook.com/apps">Facebook Application List</a> page.', 'sfc'); ?></li>
<li><?php _e('Chọn ứng dụng bạn dùng để tích hợp facebook vào wordpress sau đó copy App Id và App secret key ở đó dán vào khung dưới đây.', 'sfc'); ?></li>
</ol>
<?php
	}
}
...
<p><?php _e('Nếu bạn dùng Fanpage bạn có thể điền ID vào đây.Lấy Id của page bằng cách click vào Edit page và lấy ID của page từ Url', 'sfc'); ?></p>
<?php
...
?>
<p><?php _e('Vui lòng chọn các tính năng bạn muốn dùng!', 'sfc'); ?></p>
<?php
}
...
function sfc_default_image() {
	$options = get_option('sfc_options');
	?>
	<p><label><?php _e('Chọn ảnh đại diện cho bài viết của bạn khi được share lên facebook để thay thế khi không có ảnh nào tìm thấy từ bài viết.','sfc'); ?><br />
...
function sfc_default_description() {
	$options = get_option('sfc_options');
	?>
	<p><label><?php _e('Chọn 1 đoạn miêu tả ngắn gọn cho bài viết khi share trên facebook.Đoạn miêu tả này sẽ được sử dụng khi không có nội dung bài viết.','sfc'); ?><br />
...
function _sfc_pointer() {
	$pointer_content  = '<h3>' . __('Hướng dẫn sử dụng!', 'sfc') . '</h3>';
	$pointer_content .= '<p>' . __('Click vào hướng dẫn để xem cách dùng plugin này.','sfc') . '</p>';
?>
<script type="text/javascript">
//<![CDATA[
